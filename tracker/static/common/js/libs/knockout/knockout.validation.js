...
ko.validation.rules['validObject'] = {
    validator: function (val, params) {
        var id_attr = params[0];
        var arr = params[1]();

        if (val) {
            var all_ok = false;
            _.each(arr, function (item) {
                if (!all_ok && item[id_attr] == val) {
                    all_ok = true;
                }
            });
            return all_ok;
    ...
    message: 'Uncorrect value. Use searching'
};

ko.validation.rules['requiredAttr'] = {
    validator: function (arr, attr) {
        var all_ok = true;
        if (_.isArray(arr)) {
            ...
            return arr.length && all_ok;
        }
        return all_ok;
    },
    message: 'Значения должны быть заданы'
};

ko.validation.rules['smallerOrEqual'] = {
    validator: function (val, otherVal) {
        var otherVal = ko.utils.unwrapObservable(otherVal);
        return val <= otherVal;
    },
    message: '!'
};
